...
namespace Yubio.Server
{
    internal class LinkParser
    {
            ...
            for (int i = 1; i <= NumberOfChapters; i++)
            {
                ...
                if (IsValidURL(url, out var response))
                {
                    ...
                    foreach (Match match in regeex)
                    {
                        ...
                        var valid = IsValidURL(f.link, out var r);
                        Console.WriteLine($"Checking links on {f.link}...{valid}");
                        if (!valid)
                        {
        ...
        public static bool IsValidURL(string url, out string response)
        {
    ...
    internal class LectioParser
    {
        ...
        private static List<Team> GetTeam(School school)
        {
            ...
            foreach (Match match in b.Matches(response))
            {
                ...
                var team = new Team($@"/lectio/{school.Id}/studieplan/uvb_hold_off.aspx\?holdid={holdId}", @class);
                teams.Add(team);
            }
